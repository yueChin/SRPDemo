...
//#define TRANSFER_GI_DATA(intput,output) output.lightMapUV = input.lightMapUV;
#define TRANSFER_GI_DATA(input,output) output.lightMapUV = input.lightMapUV * unity_LightmapST.xy + unity_LightmapST.zw;
#define GI_FRAGMENT_DATA(intput) intput.lightMapUV;
...
float3 SampleEnviroment(Surface surfaceWS,BRDF brdf)
{
    ...
    float4 enviroment = SAMPLE_TEXTURECUBE_LOD(unity_SpecCube0,samplerunity_SpecCube0,uvw,0);
